...
public abstract class AbstractMessageManager<M extends Writable> implements
  ...
  protected  HashMap<InetSocketAddress, MessageQueue<M>> outgoingQueues = new HashMap<InetSocketAddress, MessageQueue<M>>();
  protected  HashMap<InetSocketAddress, MessageQueue<M>> tempOutgoingQueues = new HashMap<InetSocketAddress, MessageQueue<M>>();
  protected MessageQueue<M> localQueue;
  // for local messages, will not go through RPC
  protected MessageQueue<M> localMsgQueue;
  protected MessageQueue<M> tempLocalMsgQueue;
...
/*
   * (non-Javadoc)
   * @see org.apache.hama.bsp.message.MessageManager#getCurrentLocalMessage()
   */
  @Override
  public final M getCurrentLocalMessage() throws IOException {
    return tempLocalMsgQueue.poll();
  }
  ...
  public final int getNumCurrentMessages() {
    return localQueue.size() + localMsgQueue.size();
  }
...
/*********************my code here ********************************************/
  ...
  public void prepareSend(){
    tempOutgoingQueues.clear();
    HashMap<InetSocketAddress, MessageQueue<M>> temp = tempOutgoingQueues;
    tempOutgoingQueues = outgoingQueues;
    outgoingQueues = temp;
    //numLocalMsgs = localMsgQueue.size();
    tempLocalMsgQueue = localMsgQueue;
    tempLocalMsgQueue.prepareRead();
    localMsgQueue = getQueue();
  }
  ...
  public final Iterator<Entry<InetSocketAddress, MessageQueue<M>>> getMessageIterator() {
    if(tempOutgoingQueues.size() == 0){
      return this.outgoingQueues.entrySet().iterator();
    }
    else{
      return this.tempOutgoingQueues.entrySet().iterator();
    }
