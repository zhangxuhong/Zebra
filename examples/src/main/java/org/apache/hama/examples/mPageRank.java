...
/**
 * Real mpagerank with dangling node contribution.
 */
public class mPageRank {

  public static class mPageRankVertex extends
    ...
    //@Override
    public void partialCompute(Iterator<DoubleWritable> messages)throws IOException{
      partialResult = 0;
      if (messages.hasNext()) {
        partial = true;
    ...
    public void setup(Configuration conf) {
      String val = conf.get("hama.mpagerank.alpha");
      if (val != null) {
    ...
    public void compute(Iterator<DoubleWritable> messages) throws IOException {
      ...
      if (this.getSuperstepCount() == 0) {
        ...
        double lastvalue = this.getValue().get();
        double sum = 0;
        if(partial){
        ...
        double newvalue = (1.0d - DAMPING_FACTOR) / this.getNumVertices() + (DAMPING_FACTOR * sum);
        this.setValue(new DoubleWritable(newvalue));

      // if we have not reached our global error yet, then proceed.
        double localError = Math.abs(newvalue - lastvalue);
        if ((MAXIMUM_CONVERGENCE_ERROR > localError) && (this.getSuperstepCount() > 2)) {
          voteToHalt();
          return;
        }
      }
  ...
  public static class mPagerankTextReader extends
  ...
  public static GraphJob createJob(String[] args, HamaConfiguration conf)
      throws IOException {
    GraphJob pageJob = new GraphJob(conf, mPageRank.class);
    pageJob.setJobName("mPagerank");

    pageJob.setVertexClass(mPageRankVertex.class);
    pageJob.setInputPath(new Path(args[0]));
    pageJob.setOutputPath(new Path(args[1]));

    // set the defaults
    pageJob.setMaxIteration(30);
    pageJob.set("hama.mpagerank.alpha", "0.85");
    ...
    if (args.length >= 3)
      pageJob.set("hama.mpagerank.alpha", args[2]);
    ...
    pageJob.setVertexInputReaderClass(mPagerankTextReader.class);
    pageJob.setPartitioner(MetisPartitioner.class);
