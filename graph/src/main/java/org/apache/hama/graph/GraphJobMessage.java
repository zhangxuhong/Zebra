  ...
  public void readFields(DataInput in) throws IOException {
    ...
    if (isVertexMessage()) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        Edge<Writable, Writable> temp = new Edge<Writable, Writable>(edgeVertexID, edgeValue);
        temp.setPartitionID(partitionID);
        vertex.getEdges().add(temp);
      }
      this.vertex = vertex;
    } else if (isVerticesSizeMessage()) {
