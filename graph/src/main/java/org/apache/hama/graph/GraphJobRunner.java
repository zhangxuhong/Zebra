...
public final class GraphJobRunner<V extends Writable, E extends Writable, M extends Writable>
  ...
  private List<Vertex<V, E, M>> remoteVertices = new ArrayList<Vertex<V, E, M>>();
  ...
  private Thread LocalComputeThread = new LocalCompute();
  ...
  public final void bsp(
    ...
    while (updated && !((maxIteration > 0) && iteration > maxIteration)) {
      ...
      peer.sync();
      now =  new Date();
      LOG.info("iteration " + Long.toString(iteration) + " message parsing starts at: " + dateFormat.format(now));
      ...
      //  break;aggregator
      //}
      // only after local computation finish, start to do main superstep
      now =  new Date();
      LOG.info("iteration " + Long.toString(iteration) + " message parsing ends at: " + dateFormat.format(now));
      LocalComputeThread.join();
      now = new Date();
      LOG.info("iteration " + Long.toString(iteration) + " compute starts at: " + dateFormat.format(now));
      // loop over vertices and do their computation
      doSuperstep(messages, peer);
      now = new Date();
      LOG.info("iteration " + Long.toString(iteration-1) + " computation ends at: " + dateFormat.format(now));
      peer.prepareSend();
      // before start next superstep for local vertex computation, copy current outgoing queue to tmp outgoing queue
//***************************************start a new thread to compute local vertices ****
      if (isMasterTask(peer)) {
        peer.getCounter(GraphJobCounter.ITERATIONS).increment(1);
      }
      globalUpdateCounts = 0;
      activeVertices = 0;
      if(updated && !(iteration > maxIteration)){
        LocalComputeThread = new LocalCompute();
        LocalComputeThread.start();
      }      
    }
  ...
  private void doSuperstep(Map<V, List<M>> messages,
    ...
    if(iteration == 1){
      ...
      LOG.info("remoteVertices: " + Long.toString(remoteVertices.size()));
      LOG.info("localVertices: " + Long.toString(vertices.size()-remoteVertices.size()));
    }
    else{
      for (Vertex<V, E, M> vertex : remoteVertices) {
        List<M> msgs = messages.get(vertex.getVertexID());
  ...
  private void doInitialSuperstep(
    ...
    Date now;
    now = new Date();
    LOG.info("iteration " + Long.toString(iteration) + " computation starts at: " + dateFormat.format(now));
    for (Vertex<V, E, M> vertex : vertices) {
    ...
    now = new Date();
    LOG.info("iteration " + Long.toString(iteration) + " computation ends at: " + dateFormat.format(now));
    iteration++;
    globalUpdateCounts = 0;
    peer.prepareSend();
    LocalComputeThread.start();
  ...
  private void loadVertices(
    ...
    while ((next = peer.readNext()) != null) {
      ...
      if (runtimePartitioning) {
        ...
        /******************************here mark whether is local vertex by loop over all the edge destination id***/
        List<Edge<V, E>> outEdges = vertex.getEdges();
        if(outEdges != null){
          for (Edge<V, E> e : outEdges) {
            int id = partitioner.getPartition(e.getDestinationVertexID(), vertex.getValue(), peer.getNumPeers());
            e.setPartitionID(new IntWritable(id));
          }
        }
       /********************************************************************************/
...
/********************************************************************************************************/
  class LocalCompute extends Thread{
    public void run(){
      try {
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        
        final Map<V, List<M>> localMessages = parseLocalMessages(peer);
        doLocalSuperstep(localMessages, peer);
        Date now = new Date();
        LOG.info("iteration " + Long.toString(iteration) + " local computation ends at: " + dateFormat.format(now));

      }catch (Exception e){
        LOG.debug("LocalCompute Thread failed with " +
  ...
  /**
   * Parses the local messages in every superstep and does actions according to flags
   * in the messages.
   * 
   * @return a map that contains messages pro vertex.
   */
  @SuppressWarnings("unchecked")
  private Map<V, List<M>> parseLocalMessages(
      BSPPeer<Writable, Writable, Writable, Writable, GraphJobMessage> peer)
      throws IOException {
    GraphJobMessage msg = null;
    final Map<V, List<M>> msgMap = new HashMap<V, List<M>>();
    while ((msg = peer.getCurrentLocalMessage()) != null) {
      // either this is a vertex message or a directive that must be read
      // as map
      if (msg.isVertexMessage()) {
        final V vertexID = (V) msg.getVertexId();
  ...
  private void doLocalSuperstep(Map<V, List<M>> messages,
      BSPPeer<Writable, Writable, Writable, Writable, GraphJobMessage> peer)
      throws IOException {
    for (Vertex<V, E, M> vertex : vertices) {
      List<M> msgs = messages.get(vertex.getVertexID());
      ...
      if (!vertex.isHalted()) {
        ...
        if(iteration == 1){
          vertex.partialCompute(msgs.iterator());
        } 
        else{
          if(vertex.isLocal()){
            vertex.compute(msgs.iterator());
